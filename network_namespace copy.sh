...
# TYPE=one_network
TYPE=different_networks

echo 1 > /proc/sys/net/ipv4/conf/all/forwarding

if [ "$TYPE" == "one_network" ]; then
    echo "Вариант при котором точки доступа в одной сети"
    if [ "$CON" == "up" ]; then
...
if [ "$TYPE" == "different_networks" ]; then
    echo "Вариант при котором точки доступа в разных сетях"
    if [ "$CON" == "up" ]; then
        ...
        ip netns exec ns1 ip addr add 10.0.10.1/24 dev br0_ns1
        ip netns exec ns1 ip link set br0_ns1 up
        
        ip netns exec ns2 ip addr add 10.0.20.1/24 dev br0_ns2
        ip netns exec ns2 ip link set br0_ns2 up
        ...
        # ip netns exec ns1 ip addr add 10.0.10.5/24 dev veth0_ns1
        # ip addr add 10.0.10.1/24 dev veth1_ns1

        #   routes
        #ip netns exec ns1 ip route add 10.0.10.0/24 via 10.0.30.1
        #ip netns exec ns2 ip route add 10.0.20.0/24 via 10.0.30.1

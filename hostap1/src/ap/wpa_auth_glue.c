				...
				size_t len)
{
	...
	/*wpa_printf(MSG_DEBUG, "[%s:%d][DEBUG] FT: RRB received packet " MACSTR " -> "
		   MACSTR" proto=%d, %x %x %x %d", __func__, __LINE__,
		   MAC2STR(ethhdr->h_source), MAC2STR(ethhdr->h_dest), ethhdr->h_proto,
		   ETH_P_IP, htons(ETH_P_IP), ntohs(ETH_P_IP), ethhdr->h_proto == htons(ETH_P_IP));
	*/
#ifdef CONFIG_IEEE80211R_AP_L2_DIFFERENT_DOMAINS
	...
	if(ethhdr->h_proto == htons(ETH_P_IP)) {
		...
		// wpa_printf(MSG_DEBUG, "[%s:%d][DEBUG] src ip: %s, s_addr: %u, dd: %u, proto: %d - %d - %d",
		// 	__func__, __LINE__, ip_str, ip_header->saddr,
		// 	hapd->conf->ft_peer_list[0].sin_addr.s_addr, ip_header->protocol, htons(ip_header->protocol), ntohs(ip_header->protocol));

		if(ip_header->saddr == hapd->conf->ft_peer_list[0].sin_addr.s_addr && ip_header->protocol == IPPROTO_UDP) {
			// struct udphdr *udp = (struct udphdr *)(ip_header + sizeof(*ip_header));
			struct udphdr *udp = (struct udphdr *) ((u8 *)ip_header + ip_header->ihl * 4);
			wpa_printf(MSG_DEBUG, "[%s:%d][DEBUG] RRB L3 %s, ihl = %d, len = %d, ports: %d->%d ct:%d, len data = %d",
				__func__, __LINE__, ip_str, ip_header->ihl, len - sizeof(*ethhdr) - sizeof(*ip_header),
				ntohs(udp->source), ntohs(udp->dest), htons(udp->dest), htons(udp->len));
			// wpa_ft_rrb_rx(hapd, ethhdr->h_source, buf + sizeof(*ethhdr) + sizeof(*ip_header),
		    //   len - sizeof(*ethhdr));
			if(udp && udp->dest == htons(PORT_UDP_FT_ROAMING)) {
				wpa_printf(MSG_DEBUG, "[%s:%d][DEBUG] RRB L4, len = %d, check = %d",
					__func__, __LINE__, htons(udp->len) - sizeof(struct udphdr), udp->check);
	
				wpa_ft_rrb_rx(hapd, hapd->conf->ft_peer_list_mac, (u8*)udp + sizeof(struct udphdr),
		    	  htons(udp->len) - sizeof(struct udphdr));
			}
			// wpa_ft_rrb_rx(hapd, hapd->conf->ft_peer_list_mac, buf + sizeof(*ethhdr) + sizeof(*ip_header),
		    //   len - sizeof(*ethhdr) - sizeof(*ip_header));
		}
		return;
	}
#endif
